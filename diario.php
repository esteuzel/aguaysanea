                    ...
                    <div class="row">
                        <div class="col-md-8 col-left col-sm-8">
                            <div class="latest_articles section-category">
                                <div class="section-name"><a href="#">Consulta por diario</a></div>
                                <div class="section-content">
                                    <div class="row">
                                        <div class="col-md-6 col-sm-6">
                                            <div class="thumb3" align="center"><br><a href="#"><img src="archivo/assets/images/travel/download.gif" width="37" height="39" alt="" /></a><br><a href="#" class="title">Elija el diario</a><br><br>

                                                <form target="_blank" action="http://www.informesdiarios.com.ar/aguaysanea/archivo/result_diario.php" method="post" name="fecha" id="diario"><div class="description"><?
			  $link = mysql_connect("localhost","dario","benz"); 
			  mysql_select_db("info_nueva", $link);
		?>
                  <select name="codigo" id="diario" class="marron">
                    <?
		$result = mysql_query("SELECT * FROM dia WHERE rubro='agua' GROUP BY diario ORDER BY diario ASC", $link); 
		while ($row=mysql_fetch_array($result)){
		?>
                    <option value="<? echo $row["diario"];?>"> <? echo $row["diario"];?> </option>
                    <?
		  }
		  ?>
                  </select>
              <input name="Submit2" type="submit" value="Buscar" class="marron_mini"/>
                                                </div></form><br>Esta consulta se realiza sobre la edici&oacute;n del d&iacute;a. Para leer noticias de d&iacute;as anteriores ingrese a la secci&oacute;n archivo.<br><br>
                                            </div>
                                        </div>
                                        </div>
                                   </div>
...
?><?
						...
						$result=mysql_db_query("info_nueva","select * from dia WHERE rubro='medio ambiente' and foto!='' ORDER by Id DESC limit 0,1");
...
?>
						...
						$result=mysql_db_query("info_nueva","select * from dia WHERE rubro='general' and foto!='' ORDER by Id DESC limit 0,1");
						...
						?>
                                                    <div class="col-md-4 col-sm-4 col-xs-4"><? echo '<img src=/paginas/fotos/'.$row['foto'].' class="img-responsive" alt="se ve algo">' ?><? echo '<a target="_blank" class="title" href=http://www.informesdiarios.com.ar/aguaysanea/technology_detail.php?codigo='.$row["Id"].'>';?><? echo ''.$row["titulo"].'';?>
                                                        <div class="info"><span class="category">General</span><span
                                                                class="fa fa-circle"></span><span class="date-created"><?
						  $day = substr($fecha,8,2);
						  $month = substr($fecha,5,2);
						  $year = substr($fecha,0,4);
